 ...
 */
class TreeEncoder(
  private val network: TreeRNN,
  private val optimizer: TreeRNNOptimizer?) {
  ...
  inner class Node(val id: Int,
     ...
     */
    var encoding: NDArray
      private set

    /**
     * The errors of the vector of this node
     */
    val vectorErrors: NDArray = NDArray.zeros(shape = this.vector.shape)
     ...
     */
    private var backwardErrors: NDArray? = null

    /**
     * The errors associated to the current encoding representation of this node
     */
    internal var encodingErrors: NDArray? = null

    /**
     * The [RecurrentNeuralProcessor] to encode the left-children
     */
    internal val leftProcessor = RecurrentNeuralProcessor(this@TreeEncoder.network.leftRNN)

    /**
     * The [RecurrentNeuralProcessor] to encode the right-children
     */
    internal val rightProcessor = RecurrentNeuralProcessor(this@TreeEncoder.network.rightRNN)

    /**
     * The [FeedforwardNeuralProcessor] to take two d-dimensional vectors and returning a single d-dimensional vector
     */
    internal val concatProcessor = FeedforwardNeuralProcessor(this@TreeEncoder.network.concatNetwork)
     ...
     */
    internal fun getNodeErrors(): NDArray? = when {
     ...
     */
    internal fun encode(): NDArray = this.concatProcessor.forward(concatVectorsV(
     ...
     */
    private fun backward(errors: NDArray?) {
     ...
     */
    private fun backwardConcat(errors: NDArray): Pair<NDArray, NDArray> {
     ...
     */
    private fun encodeChildren(processor: RecurrentNeuralProcessor, head: NDArray, children: ArrayList<Node>): NDArray {
     ...
     */
    private fun backwardChildren(processor: RecurrentNeuralProcessor, children: ArrayList<Node>, errors: NDArray) {
     ...
     */
    private fun splitLeftAndRightErrors(errors: NDArray) = Pair(
   ...
   */
  fun addNode(id: Int, vector: NDArray): Node {
   ...
   */
  fun setEncodingErrors(nodeId: Int, errors: NDArray) {
    require(nodeId in this.nodes) { "Node $nodeId not found" }
