...
/**
 * A helper to train a recurrent model with sequence examples with final output.
 *
 ...
 */
class SequenceWithFinalOutputTrainer<NDArrayType: NDArray<NDArrayType>>(
   ...
   */
  override fun learnFromExample(example: SequenceExampleWithFinalOutput<NDArrayType>) {

    this.neuralProcessor.forward(example.sequenceFeatures)

    val output: DenseNDArray = this.neuralProcessor.getOutput(copy = true)
    val errors: DenseNDArray = this.lossCalculator.calculateErrors(output = output, outputGold = example.outputGold)
